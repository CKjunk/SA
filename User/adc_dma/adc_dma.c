...
__IO uint16_t SA_IMS_ADC_ConvertedValue[SAIMS_ADC_BUFFSIZE]={0};
void ADC_DMA_Config(void)
{
  ...
  // 存储器地址，实际上就是一个内部SRAM的变量	
	DMA_InitStructure.DMA_Memory0BaseAddr = (u32)SA_IMS_ADC_ConvertedValue;  
  // 数据传输方向为外设到存储器	
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;	
	
	// 缓冲区大小为，指一次传输的数据量
	DMA_InitStructure.DMA_BufferSize = SAIMS_ADC_BUFFSIZE;	
	// 外设寄存器只有一个，地址不用递增
